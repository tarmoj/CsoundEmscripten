...
CsoundObj = function()
{
	...
	var _listControlChannels = cwrap('CsoundObj_listControlChannels', 'string', 'number');
	var _enumerateControlChannels = cwrap('CsoundObj_enumerateControlChannels', 'number', 'number');
	var _getControlChannelName = cwrap('CsoundObj_getControlChannelName', 'string', 'number', 'number');
	...
	var setControlChannelValue = function(controlChannel, value)
	{
		controlChannel.value = value;
		controlChannel.updated = true;
	}

	this.setNamedControlChannelValue = function(channelName, value)
	{
		channelCount = csoundControlChannels.length;
		...
		//if (channelName == csoundControlChannels[i].name) {

		//setControlChannelValue(csoundControlChannels[i], value);
		//}
		//}
	}
	...
	this.listControlChannels = function()
	{
		var stuff =_listControlChannels(_self);
		console.log(stuff);
	}

	this.enumerateControlChannels = function()
	{
		return _enumerateControlChannels(_self);
	}

	this.getControlChannelName = function(channel)
	{
		return _getControlChannelName(_self, channel);
	}

	this.getControlChannelList = function()
	{
		csoundControlChannels = [];
		channelCount = this.enumerateControlChannels();
		for (i = 0; i < channelCount; ++i) {

			this.addControlChannel(this.getControlChannelName(i), 0);
		}
	}

	this.getControlChannels = function()
	{
		return csoundControlChannels;
	}
